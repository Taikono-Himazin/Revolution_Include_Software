...
#define Accel_Z -1458
#else
#define HMC_X -71//B:188 C:143 D:129
#define HMC_Y -279//B:60 C:71 D:89
#endif
...
void moter(uint8_t Force, int16_t Degree, bool PID) { //一応解読したがいじれるほどはわからん。とりあえず同じ形ならそのままいこう
	...
	if ((Gyro > 250 || Gyro < 110) && Errer_Flag_Status == 0) {
		if (Reset_Count >= 100) {
			Errer_Flag = true;
			LED(LED_R);
		}
...
inline void Motion_System(uint8_t Force, int16_t Degree) { //挙動制御 Force=IR_F Degree=IR_D
	...
	bool Ball2 = analogRead(A6) >= 1000;//後ろ
	bool Ball1 = false;// analogRead(A7) >= 850;//前
	if (Ball2 || Ball1) {
		LED(LED_M);
	}
	else {
...
#define Servo2_Dri Dri2_Power=180;Dri1_Power=0
	...
	if (Force != 0) {  // Ball Found      //ここから挙動制御                       //a
		...
		//		if ((265 <= Degree) && (Degree <= 275)) {
		//			M_Degree = 278;
		//			M_Force = M_P - 90;
		//			Servo2_Dri;
		//		}
		//		else if ((275 < Degree) && (Degree < 285)) {					//5
		//			M_Degree = Degree + 20;
		//			M_Force = M_P - 60;
		//			Servo2_Dri;
		//		}
		...
		//			Servo2_idel;
		//		}
		//		else if ((255 <= Degree) && (Degree < 265)) {
		//			M_Degree = Degree - 20;
		//			M_Force = M_P - 60;
		//			Servo2_Dri;
		//		}
		//
		//		if (Force <= 380 && Dri1_Power > 150) {
		//			Dri1_Power = 90;
		//		}
		//
		//		if (M_Degree < 0) {
		...
		//			Ball_Count = 0;
		//			while (bitRead(LINE_Status, 4) == 0 && Ball_Count < 100 && Ball2)
		//			{
		...
		//		}
		//
		//		uint16_t i = HC_Get(HC_L), j = HC_Get(HC_R);
		//		if ((i < 45 && i != 0) || (j < 45 && j != 0)) {
		//			if (M_Force > 60 + 60) {
		//				M_Force -= 60;
		//			}
		//		}
	...
	myServo1.write(0);//Dri1_Power);
	myServo2.write(0);// Dri2_Power);
}
...
void Spin(bool D = true) {
	...
	uint16_t j = 0;
	while (j < 2500 && bitRead(LINE_Status, 4) == 0)
	{
		LINE_Get();
		j++;
	}
	...
	j = 0;
	while (j < 1000 && bitRead(LINE_Status, 4) == 0)
	{
		LINE_Get();
		j++;
	}
