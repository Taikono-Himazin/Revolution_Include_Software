...
#define SPEAKER A0
#define BEEP 100
#define Old_Persent 0  //1以下！ Moterの過去の値の割合 
#define L_sw 4 
#define D_sw 7
#define R_sw 10
#define LED_L A3
...
inline void Motion_System(uint8_t Force, int16_t Degree) { //挙動制御 Force=IR_F Degree=IR_D
	...
	bool Ball1 = analogRead(A7) >= 850;//前
	if (Ball2 || Ball1) {
		LED(LED_M);
	}
	else {
...
#define Servo1_idel Dri2_Power=0;Dri1_Power=85
#define Servo2_idel Dri1_Power=0;Dri2_Power=65
...
#define Servo2_Dri Dri2_Power=180;Dri1_Power=0
	...
	if (Force != 0) {  // Ball Found      //ここから挙動制御                       //a
		...
		else if ((270 < Degree) && (Degree < 285)) {					//5
			M_Degree = 275;
			M_Force = M_P - 80;
			Servo2_Dri;
		}
		...
		if ((Degree > 260 && Degree < 280) && Ball2 == true) {
			Ball_Count++;
			Count_Reset = 30;
			M_Degree = 90;
			M_Force = 0;
			...
			if (i < 100 && i != 0 && Ball_Count >= 40) {
				uint16_t j = 0;
				while ((IR_D > 260 && IR_D < 280) && Ball2 == false && j < 110 && IR_F != 0)
				{
					IR_Get();
					j++;
					moter(0, 0);
		...
		Dri2_Power = 0;
		Ball_Count = 0;
		//uint32_t i;/*= HC_Get(HC_F); //Go Home
		//if (i<90 && i != 0) {
		//	M_Degree = 280;
		//	M_Force = 150;
		//}
...
void Gryo_Start() {
	...
	mpu.setXGyroOffset(-1582);
	mpu.setYGyroOffset(-644);
...
void HMC_Start() {
	...
	HMC.setOffset(-139, -313);	// Set calibration offset. See HMC5883L_calibration.ino
}
