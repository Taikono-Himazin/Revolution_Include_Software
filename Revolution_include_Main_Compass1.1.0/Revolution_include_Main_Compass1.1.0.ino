...
int16_t Compass, Compass_Now, Compass_sa, PID_MotorPWM[4];
...
m1=m1-PID_MotorPWM[0];
m2=m2-PID_MotorPWM[1];
m3=m3-PID_MotorPWM[2];
m4=m4-PID_MotorPWM[3];
...
int16_t CompassGet_5883()
{
  ...
  Compass_Now = degrees(heading); // Convert to degrees

  Compass = Compass_Now + Compass_sa;
...
void Motor_Compass()
{
  ...
  for (int i = 0; i < 4; i++) {
    PID_MotorPWM[i] -= Output;
    PID_MotorPWM[i] = constrain(PID_MotorPWM[i], -255, 255);
  }
}
