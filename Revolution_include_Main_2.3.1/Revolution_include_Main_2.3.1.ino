...
#define Servo2_Dri Dri2_Power=180;Dri1_Power=85
	...
	if (Force != 0) {  // Ball Found      //ここから挙動制御                       //a
		...
		if (LINE_R != 0) {//右
			...
			else if (bitRead(LINE_Status, 3) == 1) {//前
				M_Degree = 225;
			}
			else {
				M_Degree = 180;
...
#define LINE_EscapeCount 10 //LINEの逃げるカウント
	...
	/*		uint32_t HC_val;				//超音波使用のLINE反応
			if (LINE_R != 0) {//右
				HC_val = HC_Get_OP(HC_R);
				if (HC_val < R - 2 && HC_val != 0) {
					M_Force = 255;
					M_Degree = 180;
				}
				else {
					if (!(Degree > 270 && Degree < 90)) {
						M_Degree = 180;//ちょっと厳しめに
					}
				}
				LINE_R--;
			}
			if (LINE_B != 0) {//後ろ
				HC_val = HC_Get_OP(HC_B);
				if (HC_val < B - 2&& HC_val != 0) {
					M_Force = 255;
					M_Degree = 90;
				}
				else {
			...
			if (LINE_L != 0) {//左
				HC_val = HC_Get_OP(HC_L);
				if (HC_val < L - 2 && HC_val != 0) {
					M_Force = 255;
					M_Degree = 0;
				}
				else {
			...
			if (LINE_F != 0) {//前
				HC_val = HC_Get_OP(HC_F);
				if (HC_val < F - 2 && HC_val != 0) {
					M_Force = 255;
					M_Degree = 270;
				}
				else
...
uint32_t HC_Get_OP(uint8_t pin) {
	pinMode(pin, OUTPUT);
	digitalWrite(pin, LOW);
	delayMicroseconds(2);
	digitalWrite(pin, HIGH);
...
void HMC_Start() {
	...
	HMC.setOffset(67, -35);	// Set calibration offset. See HMC5883L_calibration.ino
}
#endif
